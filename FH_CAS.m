...
conf.size=[96,96];                       % Size of HR image
pca_conf.DR=40;                          % PCA dim of HR residual
pca_conf.DL=40;                          % PCA dim of LR residual
cca_conf.D=30;                           % Final dim by CCA
...
step1.nCluster=5;                        % Number of clusters in step1
...
step1.ksvd_conf.memusage = 'normal';     % higher usage doesn't fit...
...
[cidx,cen]=kmeans(IpLa',step1.nCluster);
cen=cen';
...
%% SR
IS=zeros(conf.size(1),conf.size(2),size(OtL,2));
PSNR=zeros(size(OtL,2),1);
SSIM=zeros(size(OtL,2),1);
for i = 1:size(OtL,3)
    i
    TL=OtL(:,:,i);
    TH=OtH(:,:,i);
    ...
    for j =1:prow*pcol
        ...
        [ctr,~,~] = neighbor2(ctl,DicL{class},DicR{class},300);
